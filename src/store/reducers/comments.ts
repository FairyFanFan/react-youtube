...
function reduceWatchDetails(responses: any, videoId: any, prevState: any) {
  const commentThreadResponse = responses.find((res: any) => res.result.kind === COMMENT_THREAD_LIST_RESPONSE);
  return reduceCommentThread(commentThreadResponse.result, videoId, prevState);
}

function reduceCommentThread(response: any, videoId: any, prevState: any) {
  if (!response) {
    return prevState;
  }
  const newComments = response.items.reduce((acc: any, item: any) => {
    acc[item.id] = item;
    return acc;
  }, {});
...
* Selectors
 */
const getCommentIdsForVideo = (state: any, videoId: any) => {
...
export const getCommentsForVideo = createSelector(
  ...
  (commentIds, allComments) => {
    return commentIds.map((commentId: any) => allComments[commentId]);
  }
);

const getComment = (state: any, location: any) => {
